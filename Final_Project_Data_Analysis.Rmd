...
```{r}
plot(mod_knn2)
#20 folds is my best performing model 
```
...
```{r}
rattle::fancyRpartPlot(mod_cart2$finalModel)
```
...
permute_imp_plot <- 
  vip(mod_rf$finalModel,
      ...
      train = training_mod2 %>% select(-Outcome),
...
#Making the map with fixed width bins 
...
poverty_map <- tm_shape(PDM_map_dat2,
  ...
  tm_shape(states,
           projection = us.proj) +
  tm_borders() +
  tm_layout(main.title = "Poverty Rate in Counties Receiving Pre-Disaster Mitigation Grants",
            legend.title.size = .8,
...
```{r}
#Given the results of my models, I want to make a few more maps: One showing the concentration of Asian households in PDM grant counties vs. the US as a whole (since that was such a strong determinant in my rf model); one showing the concentration of Black households in PDM grant counties vs. the US as a whole (since there was a negative relationship in my rf model); and one showing the disaster count by counties receiving PDM grants, since that was a strong determinant for my second rf model showing recipients of large grants.
...
Grant_asian_map <- tm_shape(ACS_asian_map2,
  ...
  tm_fill("PCT_ASIAN",
          palette = "PuBuGn",
          title = "Percent Asian Population",
          style = "jenks") + 
  tm_shape(states,
           projection = us.proj) +
  tm_borders() +
  tm_layout(main.title = "Percent of population identifying as Asian in PDM-grant counties",
            legend.title.size = .8,
            legend.text.size = 0.5,
            legend.position = c("LEFT", "BOTTOM"),
            main.title.size = .8,
...
All_asian_map <- tm_shape(ACS_asian_map,
  ...
  tm_shape(states,
           projection = us.proj) +
  tm_borders() +
  tm_layout(main.title = "Percent of population identifying as Asian in all US counties",
            legend.title.size = .8,
            legend.text.size = 0.5,
            legend.position = c("LEFT", "BOTTOM"),
            main.title.size = .8,
...
Disaster_count_map <- tm_shape(PDM_map_disasters2,
  ...
  tm_shape(states,
           projection = us.proj) +
  tm_borders() +
  tm_layout(main.title = "Number of Disasters since 2000 in PDM-grant counties",
            legend.title.size = .8,
            legend.text.size = 0.5,
            legend.position = c("LEFT", "BOTTOM"),
            main.title.size = .8,
...
All_disaster_count_map <- tm_shape(All_map_disasters2,
  ...
  tm_shape(states,
           projection = us.proj) +
  tm_borders() +
  tm_layout(main.title = "Number of Disasters since 2000 in all US counties",
            legend.title.size = .8,
            legend.text.size = 0.5,
            legend.position = c("LEFT", "BOTTOM"),
            main.title.size = .8,
...
```{r}
#Creating a map of % Black individuals in PDM-grant counties 

ACS_black_map <- get_acs(geography = "county", variables = "DP05_0038PE", output = "tidy", geometry = TRUE, shift_geo = TRUE, key = "4b641703e32072356c733cd79067e522979bc17d") %>% 
    select(GEOID, PCT_BLACK = estimate, geometry)

#Joining my Black ACS data to my PDM map data; I only want to keep 
#rows that match on GEOID so I am using an inner_join

ACS_black_map2 <- inner_join(ACS_black_map, PDM_map_dat, by = "GEOID")

#I can still use the state FIPS and aggregated county work-around to get outlines for HI and AK from above

#Making the map with jenks  bins 

Grant_black_map <- tm_shape(ACS_black_map2,
                     projection = us.proj) +
  tm_fill("PCT_BLACK",
          palette = "PuBuGn",
  ...
  tm_shape(states,
           projection = us.proj) +
  tm_borders() +
  tm_layout(main.title = "Percent of population identifying as Black in PDM-grant counties",
            legend.title.size = .8,
            legend.text.size = 0.5,
            legend.position = c("LEFT", "BOTTOM"),
            main.title.size = .8,
...
#Saving the map
tmap_save(Grant_black_map, "pct_black_PDM_counties.png", width = 1920, height = 1080, asp = 0)

#Creating my map of % Black individuals in all counties

#Making the map with jenks bins 

All_black_map <- tm_shape(ACS_black_map,
                     projection = us.proj) +
  tm_fill("PCT_BLACK",
          palette = "PuBuGn",
          title = "Percent Black Population",
  ...
  tm_shape(states,
           projection = us.proj) +
  tm_borders() +
  tm_layout(main.title = "Percent of population identifying as Black in all US counties",
            legend.title.size = .8,
            legend.text.size = 0.5,
            legend.position = c("LEFT", "BOTTOM"),
            main.title.size = .8,
...
#Saving the map
tmap_save(All_black_map, "pct_black_all_counties.png", width = 1920, height = 1080, asp = 0)
