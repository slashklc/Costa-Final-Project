...
###Machine learning 2: Splitting my outcomes

```{r}
#I'm curious if there's any difference between the characteristics of counties receiving small grants (<$100,000) and those receiving larger grants (up to $32 million). 
...
dat_all_split <- dat_all_split %>% 
  mutate(grant_level = ifelse(ProjectAmount > 100000, "high", "low"))
...
trainingGrant <- trainingGrant %>% 
  select(GEOID, GrantLevel, ProjectAmount, DisasterCount, POVERTY_RATE, PCT_WHITE, PCT_BLACK, PCT_NATIVE, PCT_ASIAN, PCT_TWO_OR_MORE, PCT_HISPANIC) %>% 
  mutate_at(., vars(DisasterCount), funs(ifelse(is.na(.), 0, .)))
```
...
#I don't want to use GEOID as a predictor, but also don't want to disassociate the GEOID from the data, 
#so selecting the other predictors only; also don't want to use ProjectAmount
mod_rf_grant <- train(GrantLevel ~ DisasterCount + POVERTY_RATE + PCT_WHITE + 
                             PCT_BLACK + PCT_NATIVE + PCT_ASIAN + PCT_TWO_OR_MORE + PCT_HISPANIC,
...
poverty_map <- tm_shape(PDM_map_dat,
                     projection = us.proj) +
  tm_fill("PovertyRate",
          palette = "PuBuGn",
          title = "Poverty Rate") + 
